...
/**
 * Event for when access to a assign is prevented by this subplugin.
 *
...
/**
 * Event for when access to an assign is prevented by this subplugin.
 *
 * @package    assignsubmission_seb
 * @copyright  2024 Universidad de Málaga
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
...
class access_prevented extends base {
     ...
     * @return base
     */
    public static function create_strict(
        ...
        return self::create([
            'userid' => $USER->id,
            'objectid' => settings_provider::get_assign_id($accessmanager->get_assign()),
            'courseid' => $accessmanager->get_assign()->get_course()->id,
            'context' => $accessmanager->get_assign()->get_context(),
            'other' => $other,
        ]);
    }
    ...
    public static function get_name() {
        return get_string('event:accessprevented', 'assignsubmission_seb');
    }
    ...
    public function get_description() {
        $description = "The user with id '$this->userid' has been prevented from accessing assign with id '$this->objectid' by the "
                . "Safe Exam Browser assign submission plugin. The reason was '{$this->other['reason']}'. "
