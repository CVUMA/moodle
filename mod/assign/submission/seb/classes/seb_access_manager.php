...
/**
 * Manage the access to the assign.
 *
...
class seb_access_manager {
    ...
    /** @var assign $assign An assign object containing all information pertaining to current assign. */
    private $assign;

    /** @var seb_assign_settings $assignsettings A SEB assign settings persistent object containing plugin settings */
    private $assignsettings;

    /** @var context_module $context Context of this assign activity. */
    private $context;
     ...
     *
     * @param assign $assign The details of the assign.
     */
    public function __construct(assign $assign) {
        $this->assign = $assign;
        $this->context = \context_module::instance($assign->get_course_module()->id);
        $assignid = settings_provider::get_assign_id($assign);
        $this->assignsettings = seb_assign_settings::get_by_assign_id($assignid);
        $this->validconfigkey = seb_assign_settings::get_config_key_by_assign_id($assignid);
    }
    ...
    public function validate_browser_exam_key(?string $browserexamkey = null, ?string $url = null): bool {
        ...
        $validbrowserexamkeys = $this->assignsettings->get('allowedbrowserexamkeys');
        if (!empty($validbrowserexamkeys)) {
            $validbrowserexamkeys = $this->assignsettings->split_keys($validbrowserexamkeys);
        }
    ...
    public function validate_config_key(?string $configkey = null, ?string $url = null): bool {
        ...
        if (
            $requiredtype == settings_provider::USE_SEB_NO
    ...
    /**
     * Check if Safe Exam Browser is required to access assign.
     * If assignsettings do not exist, then there is no requirement for using SEB.
     *
     * @return bool If required.
     */
    public function seb_required(): bool {
        if (!$this->assignsettings) {
            return false;
        } else {
    ...
    public function can_bypass_seb(): bool {
        return has_capability('assignsubmissionaccess/seb:bypassseb', $this->context);
    }
    ...
    /**
     * Getter for the assign object.
     *
     * @return assign Assign object.
     */
    ...
    /**
     * Check that at least one browser exam key exists in the assign settings.
     *
     * @return bool True if one or more keys are set in assign settings.
     */
    private function is_allowed_browser_examkeys_configured(): bool {
        return !empty($this->assignsettings->get('allowedbrowserexamkeys'));
    }
    ...
    /**
     * Get type of SEB usage for the assign.
     *
     * @return int
     */
    public function get_seb_use_type(): int {
        if (empty($this->assignsettings)) {
            return settings_provider::USE_SEB_NO;
        } else {
            return $this->assignsettings->get('requiresafeexambrowser');
        }
    }
    ...
    /**
     * Set session access for assign.
     *
     * @param bool $accessallowed
     */
    public function set_session_access(bool $accessallowed): void {
        global $SESSION;
        if (!isset($SESSION->assignaccess_seb_access)) {
            $SESSION->assignaccess_seb_access = [];
        }
        $SESSION->assignaccess_seb_access[$this->assign->get_course_module()->id] = $accessallowed;
    }

    /**
     * Check session access for assign if already set.
     *
     * @return bool
     */
    public function validate_session_access(): bool {
        global $SESSION;
        return !empty($SESSION->assignaccess_seb_access[$this->assign->get_course_module()->id]);
    }

    /**
     * Unset the global session access variable for this assign.
     */
    public function clear_session_access(): void {
        global $SESSION;
        unset($SESSION->assignaccess_seb_access[$this->assign->get_course_module()->id]);
    }
    ...
    public function redirect_to_seb_config_link(): void {
        ...
        $seblink = \assignsubmission_seb\link_generator::get_link($this->assign->get_course_module()->id, true, is_https());
    ...
    public function should_redirect_to_seb_config_link(): bool {
        ...
        return $this->is_using_seb()
                && get_config('assignsubmission_seb', 'autoreconfigureseb')
                && $haskeyinheader;
    }
}
