...
class helper {
    ...
    /**
     * Get seb config content for a particular assign. This method checks caps.
     *
     * @param string $cmid The course module ID for an assign with config.
    ...
    public static function get_seb_config_content(string $cmid): string {
        ...
        // Retrieve the config for assign.
        $config = seb_assign_settings::get_config_by_assign_id($cm->instance);
        if (empty($config)) {
            throw new \moodle_exception('noconfigfound', 'assignsubmission_seb', '', $cm->id);
        }
        return $config;
    }
