...
class template extends persistent {
    ...
    const TABLE = 'assignsubmission_sebtemplate';
    ...
    protected function validate_content(string $content) {
        if (helper::is_valid_seb_config($content)) {
            ...
            return new \lang_string('invalidtemplate', 'assignsubmission_seb');
        }
    }
    ...
    public function can_delete(): bool {
        ...
        if ($this->get('id')) {
            $settings = seb_assign_settings::get_records_by_templateid($this->get('id'));
