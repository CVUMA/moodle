 ...
 *
 * @module     assignsubmission_seb/validate_assign_access
...
import * as View from 'assignsubmission_seb/view';
 ...
 *
 * @param {init} cmid Value of course module id of the assign.
 * @param {boolean} autoreconfigure Value of Moodle setting: assignsubmission_seb/autoreconfigureseb.
 */
const safeExamBrowserKeysUpdated = (cmid, autoreconfigure = false) => {
    // Action opening up the assign.
    isAssignAccessValid(cmid).then((response) => {
        ...
        if (response.configkey && response.browserexamkey) {
            ...
            // If autoreconfigureseb is enabled, attempt to reconfigure page with assign settings.
 ...
 * @return {Promise}
 */
const isAssignAccessValid = (cmid) => {
    const request = {
        methodname: 'assignsubmission_seb_validate_assign_keys',
        args: {
            cmid: cmid,
...
/**
 * Reload Safe Exam Browser with current assign configuration.
 *
 * @param {init} cmid Value of course module id of the assign.
 */
const reconfigureSafeExamBrowser = (cmid) => {
    const domain = Config.wwwroot.replace(/^http/i, 'seb');
    const redirecturl = domain + '/mod/assign/submission/seb/config.php?cmid=' + cmid;
